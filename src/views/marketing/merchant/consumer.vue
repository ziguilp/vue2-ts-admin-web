 ...
 * @LastEditTime  : 2023-06-15 08:50:59
 * @FilePath      : /nls-admin/src/views/marketing/merchant/consumer.vue
 * @Description   : 
 * 
...
<template>
    <div>
        <custom-list :conf="config" ref="consumer">
...
import { getConsumerLog } from "@/api/merchant";
    ...
    private config: CustomListConf = {
        ...
        columns: [
            ...
            },
            {
                ...
                showFormatInTable: (row: any) => {
                    return `${row.remark}:${
                        row.payout_info?.apply_log?.location.address ||
                        row.payout_info?.boxcode_log ||
                        row.payout_info?.desc
                    }`;
                },
            },
            {
        ...
        onLoadData: async (searchForm: any, idata: IpageDataDto<any>) => {
            ...
            const data: any = await getConsumerLog({
                data: pdata,
